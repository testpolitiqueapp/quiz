...
const App: React.FC = () => {
  ...
  const calculateResult = useCallback(() => {
    ...
    // MISE À JOUR IMPORTANTE ICI : Ajout de 'culture-inclusive'
    ...
    quizQuestions.forEach((question, index) => {
        ...
        userCompassScores[question.axis].score += answerValue * direction * finalWeight;
        userCompassScores[question.axis].max += 2 * finalWeight;
    ...
    for (const tag in userTagScores) {
      ...
      if (userTagScores[tag].max > 0) {
        ...
        // Inversion pour les tags progressifs
    ...
    const allPartiesWithPercentages: FinalResultItem[] = PARTIES.map(partyInfo => {
      ...
      if (partyInfo.cohesion !== undefined) {
        const cohesionDifference = Math.abs(userRadicalism - partyInfo.cohesion); // Note: userRadicalism is used as a proxy for cohesion here
