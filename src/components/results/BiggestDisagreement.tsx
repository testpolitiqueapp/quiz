...
export const BiggestDisagreement: React.FC<BiggestDisagreementProps> = ({
  ...
  themeClasses,
    ...
    return (
        <GlassTile partyId={party.id} isDark={isDark} className="overflow-hidden p-0">
                ...
                <div className="flex items-center justify-center gap-3 mb-8">
                    <div className="relative group">
                        ...
                        <div className="absolute -bottom-3 -right-3 z-30">
                            <XCircle 
                                className={twMerge(
                                    "w-10 h-10 rounded-full shadow-lg ring-1",
                                    isDark 
                                      ? "text-red-300 bg-slate-800 ring-slate-700/50" 
                                      : "text-red-600 bg-white ring-slate-300/50"
                                )} 
                            />
                        </div>
                    </div>
                    ...
                    {selectedPhoto && (
                        <div className="relative group">
                            ...
                            <div 
                                className={twMerge(
                                    "relative w-32 h-32 sm:w-40 sm:h-40 rounded-3xl overflow-hidden",
                                    "shadow-xl", // ring-1 ring-white/10 a été supprimé
                                    "transition-all duration-300 transform",
                                    "hover:scale-105 hover:shadow-2xl",
                                    isDark ? 'bg-slate-800/50' : 'bg-white/80'
                                )}
                                style={{
                                    filter: 'grayscale(80%)', // Filtre gris appliqué
                                    boxShadow: `0 0 0 2px ${isDark ? 'rgba(100, 116, 139, 0.5)' : 'rgba(203, 213, 225, 0.8)'}` // Bordure neutre ajoutée
                                }}
                            >
                                <img 
                                    src={selectedPhoto} 
